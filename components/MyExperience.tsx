...
function MyExperience() {
  return (
    <section className='bg-black px-20 py-section max-sm:px-4'>
      ...
      <ul className='py-10 mt-5 flex flex-col gap-y-8 mx-8 px-6 max-sm:py-5 max-sm:px-0 max-sm:mx-0'>
        {myExperience.map((item: {src: string, alt: string, title: string, duration: string, text: string}, idx: number) => (
          <li key={idx} className={`border-zinc-500 rounded-xl border-1 py-8 px-6 pb-7 text-white  ${idx%2 === 1 ? 'bg-zinc-800' : ''} max-sm:py-[1.875rem] max-sm:px-5`}>
            <div className='flex justify-between max-sm:flex-col'>
              ...
              <span className='text-zinc-300 font-semibold max-sm:pt-[1.875rem]'>{item.duration}</span>
            </div>
