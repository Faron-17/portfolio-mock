    ...
    <section className='bg-black px-20 py-section max-sm:px-4'>
      ...
      <ul className='pt-5 mx-8 flex flex-col gap-5 max-sm:mx-0 max-sm:gap-7'>
        {myProjects.map((item: {title: string, text: string}, idx: number) => (
          <li key={idx} className={`h-[32.25rem] text-white flex items-center gap-x-10 ${idx%2 !== 0 ? 'flex-row-reverse' : ''} max-sm:flex-col max-sm:h-auto`}>
            <div className={`w-[37.125rem] max-sm:w-auto ${idx%2 != 0 ? 'flex justify-end' : ''}`}>
              <div className={`w-[33.125rem] h-[24.813rem] bg-my-project-${idx+1} rounded-2xl max-sm:w-[21.438rem] h-[24.875rem]`}></div>
            </div>
            <div className='w-[36.375rem] max-sm:w-auto max-sm:pt-6'>
              <h1>
